(Get-Content "C:\PulseAudio\etc\pulse\default.pa") -replace "load-module module-native-protocol-tcp auth-ip-acl\b.*", "load-module module-native-protocol-tcp auth-ip-acl=$(Ubuntu run "ip addr show eth0 | grep -oP '(?<=inet\s)\d+(\.\d+){3}'")" | Set-Content "C:\PulseAudio\etc\pulse\default.pa"
